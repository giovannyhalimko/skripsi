...
#### 3.3.2.3 _Magnitude Spectrum_, _High-Pass Filtering_, dan _Log Scaling_
...
Sebelum transformasi logaritmik, _magnitude spectrum_ dilewatkan melalui _Gaussian high-pass filter_ yang dirancang untuk menekan dominasi komponen frekuensi rendah di sekitar pusat peta (setelah `fftshift`). Penekanan ini penting karena pada citra wajah, energi frekuensi terkonsentrasi pada komponen DC dan frekuensi rendah, yang sebagian besar merepresentasikan struktur global wajah (bentuk, pencahayaan) dan kurang informatif untuk deteksi _deepfake_. Sebaliknya, artefak sintesis _deepfake_ — seperti _checkerboard patterns_ dan _spectral rolloff_ yang anomal — terutama termanifestasi pada komponen frekuensi menengah dan tinggi. _Mask high-pass_ Gaussian didefinisikan sebagai:

$$
H(u, v) = 1 - \exp\left(-\frac{(u - u_c)^2 + (v - v_c)^2}{2(\beta \cdot N)^2}\right) \tag{3.4}
$$

di mana $(u_c, v_c)$ adalah koordinat pusat peta frekuensi setelah `fftshift`, $N$ adalah dimensi peta (224), dan $\beta = 0{,}15$ adalah _cutoff fraction_ yang mengontrol radius penekanan ($\sigma = \beta \cdot N \approx 33{,}6$ piksel). _Magnitude_ yang telah difilter kemudian dihitung sebagai $|F'(u, v)| = |F(u, v)| \cdot H(u, v)$.

Setelah _high-pass filtering_, dilakukan transformasi logaritmik untuk mengompresi rentang dinamis yang masih sangat lebar:

$$
M_{\log}(u, v) = \log(1 + |F'(u, v)|) \tag{3.5}
$$

Fungsi $\log(1 + x)$ (dikenal sebagai `log1p`) dipilih karena dua alasan: (1) menghindari permasalahan $\log(0)$ karena $\log(1+0) = 0$, dan (2) mengompresi rentang dinamis sehingga detail pada frekuensi tinggi tidak tertutupi oleh dominasi komponen frekuensi rendah sisa. Hasil akhir berupa matriks _float32_ berukuran $224 \times 224$ dengan nilai tipikal pada rentang $[0, \sim 16]$ yang disimpan sebagai berkas `.npy` (_NumPy array_) dalam _cache_ FFT.
...
$$
F(0,0) = \sum_{x=0}^{3}\sum_{y=0}^{3} f(x,y) \cdot e^{-j2\pi(0)} = \sum_{x=0}^{3}\sum_{y=0}^{3} f(x,y) \tag{3.6}
$$
...
$$
F(0,1) = \sum_{x=0}^{3}\sum_{y=0}^{3} f(x,y) \cdot e^{-j2\pi \cdot y/4} \tag{3.7}
$$
...
$$
F(1,0) = \sum_{x=0}^{3}\sum_{y=0}^{3} f(x,y) \cdot e^{-j2\pi x/4} \tag{3.8}
$$
...
$$
F(1,1) = \sum_{x=0}^{3}\sum_{y=0}^{3} f(x,y) \cdot e^{-j2\pi(x+y)/4} \tag{3.9}
$$
...
$$
\hat{x}_{\text{fft}} = x_{\text{fft}} + \epsilon, \quad \epsilon \sim \mathcal{N}(0, \sigma^2) \tag{3.11}
$$
...
Selain injeksi _noise_, diterapkan pula **_spectral band masking_** dengan probabilitas 5%. Pada setiap _frame_ yang terpilih secara acak, sebuah pita frekuensi — baik horizontal maupun vertikal — dengan lebar acak antara 1 hingga $\max(\lfloor H/16 \rfloor, 2)$ piksel diisi dengan nilai nol:

$$
\hat{x}_{\text{fft}}[r_1 : r_1 + w, :] = 0 \quad \text{(pita horizontal)} \tag{3.12}
$$

$$
\hat{x}_{\text{fft}}[:, c_1 : c_1 + w] = 0 \quad \text{(pita vertikal)} \tag{3.13}
$$
...
$$
\mathbf{X}_1 = \begin{bmatrix} 1 & 2 & 0 \\ 3 & 1 & 2 \\ 0 & 1 & 3 \end{bmatrix}, \quad \mathbf{X}_2 = \begin{bmatrix} 2 & 0 & 1 \\ 1 & 3 & 0 \\ 2 & 1 & 1 \end{bmatrix} \tag{3.14}
$$

**Tahap 1 — _Depthwise_:** Filter $2 \times 2$ terpisah per kanal (tanpa _padding_, _stride_ = 1):

$$
\mathbf{W}_1 = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}, \quad \mathbf{W}_2 = \begin{bmatrix} 0 & 1 \\ 1 & 0 \end{bmatrix} \tag{3.15}
$$

Konvolusi kanal 1 ($\mathbf{X}_1 * \mathbf{W}_1$), posisi $(0,0)$:

$$
1 \times 1 + 2 \times 0 + 3 \times 0 + 1 \times 1 = 2 \tag{3.16}
$$
...
$$
2 \times 0 + 0 \times 1 + 1 \times 1 + 3 \times 0 = 1 \tag{3.17}
$$
...
$$
\mathbf{Z}(i,j) = 0{,}5 \times \mathbf{Y}_1(i,j) + 0{,}5 \times \mathbf{Y}_2(i,j) \tag{3.18}
$$
...
$$
\mathbf{y} = \text{MaxPool}_{2\times2}\left(\text{Conv}_{3\times3}(\mathbf{x}) + \text{Shortcut}(\mathbf{x})\right) \tag{3.19}
$$
...
Konfigurasi yang digunakan dalam penelitian ini adalah _depth_ = 5 dengan _base_channels_ = 64, menghasilkan progresi kanal $[64, 128, 256, 512, 512]$. Dua blok terakhir menggunakan jumlah kanal yang sama (512), di mana jalur pintasan pada blok kelima berupa operasi identitas karena dimensi input dan output sudah sesuai. Pemilihan _base_channels_ = 64 (alih-alih nilai _default_ 32) dilatarbelakangi oleh karakteristik input berupa _crop_ wajah dengan distribusi spektral yang relatif kaya: kapasitas representasi yang lebih besar pada tahap awal jaringan memberikan ruang yang memadai untuk menangkap variasi artefak frekuensi yang halus tanpa kehilangan informasi pada lapisan-lapisan dangkal.

Tabel 3.5 Arsitektur _Layer-by-Layer_ FreqCNN (_depth_ = 5, _base_channels_ = 64)

| _Layer_         | Tipe                                                | Dimensi Input    | Dimensi Output    | Parameter       |
| --------------- | --------------------------------------------------- | ---------------- | ----------------- | --------------- |
| FreqBlock 1     | Conv(1→64, 3×3) + BN + Shortcut(1×1) + MaxPool      | $(1, 224, 224)$  | $(64, 112, 112)$  | $\sim 900$      |
| FreqBlock 2     | Conv(64→128, 3×3) + BN + Shortcut(1×1) + MaxPool    | $(64, 112, 112)$ | $(128, 56, 56)$   | $\sim 82.400$   |
| FreqBlock 3     | Conv(128→256, 3×3) + BN + Shortcut(1×1) + MaxPool   | $(128, 56, 56)$  | $(256, 28, 28)$   | $\sim 329.000$  |
| FreqBlock 4     | Conv(256→512, 3×3) + BN + Shortcut(1×1) + MaxPool   | $(256, 28, 28)$  | $(512, 14, 14)$   | $\sim 1.313.000$|
| FreqBlock 5     | Conv(512→512, 3×3) + BN + Identity + MaxPool        | $(512, 14, 14)$  | $(512, 7, 7)$     | $\sim 2.361.000$|
| Dropout2d(0,2)  | _Spatial dropout_                                   | $(512, 7, 7)$    | $(512, 7, 7)$     | 0               |
| AdaptiveAvgPool | _Global average pooling_                            | $(512, 7, 7)$    | $(512, 1, 1)$     | 0               |
| FC 1            | Linear(512→256) + ReLU                              | $(512)$          | $(256)$           | $\sim 131.000$  |
| Dropout(0,3)    | _Dropout_                                           | $(256)$          | $(256)$           | 0               |
| FC 2            | Linear(256→1)                                       | $(256)$          | $(1)$             | $\sim 257$      |
| **Total**       |                                                     |                  |                   | **$\sim$ 4,2M** |

Meskipun jumlah parameter FreqCNN ($\sim 4{,}2$ juta) lebih besar dibandingkan konfigurasi _default_ _depth_ = 3, _base_channels_ = 32 ($\sim 130.000$), model ini tetap jauh lebih kecil dari XceptionNet ($\sim 22{,}8$ juta parameter). Kedalaman 5 blok dipilih untuk mengekstraksi fitur frekuensi yang lebih kaya — blok-blok tambahan memungkinkan model menangkap pola spektral pada berbagai tingkat abstraksi, dari artefak frekuensi rendah (seperti distribusi energi global) hingga anomali frekuensi tinggi (seperti _spectral rolloff_ dan _checkerboard patterns_). Koneksi residual memastikan bahwa pelatihan tetap stabil meskipun kedalaman jaringan bertambah.

Kedalaman dan lebar FreqCNN dapat dikonfigurasi melalui parameter `freq_depth` dan `freq_base_channels`. Jumlah kanal berlipat dua pada setiap blok hingga batas maksimum $8 \times \text{base\_channels} = 512$ untuk konfigurasi $\text{base\_channels} = 64$ yang digunakan dalam penelitian ini.
...
$$
\mathbf{X} = \begin{bmatrix} 2{,}1 & 3{,}5 & 1{,}2 & 4{,}0 \\ 0{,}8 & 2{,}3 & 3{,}1 & 1{,}5 \\ 4{,}2 & 1{,}0 & 2{,}7 & 3{,}3 \\ 1{,}6 & 3{,}8 & 0{,}5 & 2{,}9 \end{bmatrix} \tag{3.20}
$$
...
$$
z_{0,0} = 0 \cdot w_{00} + 0 \cdot w_{01} + 0 \cdot w_{02} + 0 \cdot w_{10} + 2{,}1 \cdot w_{11} + 3{,}5 \cdot w_{12} + 0 \cdot w_{20} + 0{,}8 \cdot w_{21} + 2{,}3 \cdot w_{22} \tag{3.21}
$$

**BatchNorm:** Setelah seluruh posisi dihitung, _batch normalization_ menormalisasi output:

$$
\hat{z}_i = \frac{z_i - \mu_B}{\sqrt{\sigma_B^2 + \epsilon}} \cdot \gamma + \beta \tag{3.22}
$$
...
Cabang frekuensi menggunakan bagian _backbone_ dari FreqCNN (blok konvolusional + _global average pooling_) tanpa lapisan klasifikasi. Input berupa peta _magnitude_ FFT berukuran $(1, 224, 224)$ yang telah dinormalisasi menggunakan _z-score_ per dataset. Output berupa vektor fitur berdimensi 512 (pada konfigurasi _depth_ = 5 dengan _base_channels_ = 64) yang merepresentasikan pola frekuensi dalam citra.

#### 3.4.3.3 _Projection Layers_ dan Penyeimbangan Dimensi

Tantangan utama dalam _late fusion_ adalah ketidakseimbangan dimensi antara kedua cabang. Cabang spasial menghasilkan vektor berdimensi 2.048, sedangkan cabang frekuensi 512 — rasio 4:1. Apabila kedua vektor langsung digabungkan (_concatenation_), fitur spasial akan mendominasi proses klasifikasi karena magnitudonya yang jauh lebih besar.

Untuk mengatasi hal ini, kedua cabang diproyeksikan ke dimensi yang sama ($\text{PROJ\_DIM} = 256$) melalui lapisan proyeksi:

$$
\mathbf{h}_{\text{spatial}} = \text{ReLU}(\text{BN}(\mathbf{W}_s \cdot \mathbf{f}_{\text{spatial}} + \mathbf{b}_s)) \tag{3.23}
$$

$$
\mathbf{h}_{\text{freq}} = \text{ReLU}(\text{BN}(\mathbf{W}_f \cdot \mathbf{f}_{\text{freq}} + \mathbf{b}_f)) \tag{3.24}
$$

di mana $\mathbf{W}_s \in \mathbb{R}^{256 \times 2048}$ dan $\mathbf{W}_f \in \mathbb{R}^{256 \times 512}$ adalah matriks bobot proyeksi, BN adalah _batch normalization_ 1D, dan ReLU adalah fungsi aktivasi. Setelah proyeksi, kedua cabang memiliki representasi berdimensi sama (256) yang dapat digabungkan secara seimbang.
...
$$
\mathbf{g} = \sigma(\mathbf{W}_2 \cdot \text{ReLU}(\mathbf{W}_1 \cdot \mathbf{h}_{\text{fused}} + \mathbf{b}_1) + \mathbf{b}_2) \tag{3.25}
$$

$$
\hat{\mathbf{h}}_{\text{fused}} = \mathbf{h}_{\text{fused}} \odot \mathbf{g} \tag{3.26}
$$
...
1. **Dropout(0,5)** — Regularisasi pada representasi fusi untuk mencegah _overfitting_. Tingkat _dropout_ yang relatif tinggi dipilih karena kepala klasifikasi merupakan satu-satunya komponen yang langsung memetakan fitur fusi menjadi keputusan _real_/_fake_; tanpa regularisasi yang cukup, lapisan ini cenderung _overfit_ pada pola spesifik dataset pelatihan, terutama pada ukuran sampel kecil.
2. **Linear(512 → 128)** — Reduksi dimensi.
3. **ReLU** — Fungsi aktivasi.
4. **Dropout(0,5)** — Regularisasi tambahan setelah reduksi dimensi.
...
$$
\mathbf{f}_{\text{spatial}} = [0{,}8;\ 1{,}2;\ -0{,}3;\ 0{,}5], \quad \mathbf{f}_{\text{freq}} = [0{,}4;\ -0{,}1;\ 0{,}7;\ 0{,}2] \tag{3.27}
$$
...
$$
\mathbf{h}_{\text{fused}} = [0{,}8;\ 1{,}2;\ 0;\ 0{,}5;\ 0{,}4;\ 0;\ 0{,}7;\ 0{,}2] \in \mathbb{R}^8 \tag{3.28}
$$
...
$$
\hat{\mathbf{h}}_{\text{fused}} = \mathbf{h}_{\text{fused}} \odot \mathbf{g} = [0{,}72;\ 0{,}84;\ 0;\ 0{,}40;\ 0{,}24;\ 0;\ 0{,}63;\ 0{,}10] \tag{3.29}
$$
...
| Dimensi fitur   | 2.048                      | 512                      | $256 + 256 = 512$ (terproyeksi) |
| Mekanisme fusi  | —                          | —                        | Konkatenasi + SE*Gating*        |
| Total parameter | $\sim 22{,}8$ juta         | $\sim 4{,}2$ juta        | $\sim 27{,}7$ juta              |
...
| FreqCNN*backbone*     | $(1, 224, 224)$ | 512            | 5 FreqBlock residual + GAP |
| Proyeksi spasial      | 2.048           | 256            | Linear + BN + ReLU         |
| Proyeksi frekuensi    | 512             | 256            | Linear + BN + ReLU         |
...
FFT (1,224,224) → [FreqCNN Backbone] → (512) → [Proj: Linear→BN→ReLU] → (256) ────────┘
                   (5 FreqBlocks residual)       (512→256)
-->
...
| Cabang frekuensi (_hybrid_)  | $5 \times 10^{-5}$ (base $\times$ 0,25) | Parameter FreqCNN dalam model _hybrid_              |
| _Head_ (lapisan baru)        | $2 \times 10^{-4}$ (base)             | Proyeksi, SE _gate_, _classifier_; juga FreqCNN _standalone_ |

_Learning rate_ _backbone_ yang 10 kali lebih rendah memastikan bahwa _fine-tuning_ dilakukan secara halus tanpa menghancurkan representasi visual yang telah dipelajari dari ImageNet, sementara lapisan baru mendapat _learning rate_ yang lebih tinggi untuk konvergensi yang lebih cepat. Khusus pada model _hybrid_, cabang FreqCNN diberikan _learning rate_ menengah (25% dari _base_) untuk mencegah gradien dari _branch_ frekuensi yang diinisialisasi secara acak mendominasi pembaruan bobot pada awal pelatihan, sekaligus menjaga keseimbangan kontribusi antara kedua cabang.

### 3.5.3 Penjadwalan _Learning Rate_

Penjadwalan _learning rate_ menggabungkan dua fase menggunakan `SequentialLR`:

1. **_Linear warmup_** (3 _epoch_) — _Learning rate_ dinaikkan secara linier dari 10% ke 100% dari nilai _base_. `LinearLR(start_factor=0.1, end_factor=1.0, total_iters=3)`. Fase ini mencegah pembaruan bobot yang terlalu besar pada iterasi awal, terutama penting pada _backbone pretrained_. Durasi _warmup_ disinkronkan dengan periode _backbone freezing_ (3 _epoch_) sehingga peningkatan _learning rate_ terjadi bersamaan dengan pelatihan lapisan baru, dan pada _epoch_ ke-4 saat _backbone_ dilepaskan, _learning rate_ sudah berada pada nilai _base_ penuh siap untuk fase _fine-tuning_.
2. **_Cosine annealing_** (_epoch_ ke-4 hingga selesai) — _Learning rate_ diturunkan secara halus mengikuti kurva kosinus dari nilai _base_ menuju $1 \times 10^{-6}$. `CosineAnnealingLR(T_max=max(epochs-3, 1), eta_min=1e-6)`. Penurunan yang mulus ini menghindari _shock_ akibat penurunan LR yang tiba-tiba.

Kurva _learning rate_ untuk pelatihan 30 _epoch_:

- _Epoch_ 1: $\text{LR} \approx 0{,}1 \times \text{base} = 2 \times 10^{-5}$
- _Epoch_ 2: $\text{LR} \approx 0{,}55 \times \text{base} = 1{,}1 \times 10^{-4}$
- _Epoch_ 3: $\text{LR} = 1{,}0 \times \text{base} = 2 \times 10^{-4}$
- _Epoch_ 4–30: _Cosine decay_ dari $2 \times 10^{-4}$ menuju $1 \times 10^{-6}$
...
$$
\mathcal{L} = -\frac{1}{N}\sum_{i=1}^{N}\left[w_p \cdot y_i \cdot \log(\sigma(z_i)) + (1 - y_i) \cdot \log(1 - \sigma(z_i))\right] \tag{3.30}
$$
...
$$
w_p = \frac{n_{\text{neg}}}{n_{\text{pos}}} \tag{3.31}
$$
...
$$
y'_i = y_i \times (1 - \alpha) + \alpha \times 0{,}5 \tag{3.32}
$$

Sebagai ilustrasi, dengan $\alpha = 0{,}05$, transformasi ini mengubah label biner menjadi: $0 \rightarrow 0{,}025$ dan $1 \rightarrow 0{,}975$, sehingga model tidak perlu mendorong _logit_ ke nilai ekstrem ($\pm \infty$).

Pada konfigurasi akhir yang digunakan dalam eksperimen ini, _label smoothing_ diaktifkan dengan $\alpha = 0{,}05$. Nilai _smoothing_ yang ringan ini dipilih untuk memberikan efek regularisasi yang mencegah model menjadi terlalu _overconfident_ pada label _hard_ — terutama relevan ketika _backbone_ XceptionNet _pretrained_ digabungkan dengan _head_ yang dilatih dari awal, karena kombinasi ini cenderung menghasilkan _logit_ ekstrem pada awal pelatihan. Sebaliknya, $\alpha$ dijaga cukup kecil ($\leq 0{,}05$) agar tidak melemahkan sinyal positif pada dataset berukuran kecil (mulai dari $n = 100$), di mana sinyal positif yang terlalu lemah dapat menyulitkan model membedakan kelas _real_ dan _fake_.

#### 3.5.4.3 Contoh Perhitungan BCEWithLogitsLoss

Sebagai ilustrasi, berikut ditunjukkan contoh perhitungan BCEWithLogitsLoss dengan _label smoothing_ $\alpha = 0{,}05$ (sesuai konfigurasi akhir). Misalkan _logit_ output model $z = 2{,}5$, label asli $y = 1$ (_fake_), dan $w_p = 1$ (dataset seimbang).

**Langkah 1: _Label smoothing_:**

$$
y' = 1 \times (1 - 0{,}05) + 0{,}05 \times 0{,}5 = 0{,}95 + 0{,}025 = 0{,}975 \tag{3.33}
$$

**Langkah 2: _Sigmoid_:**

$$
\sigma(2{,}5) = \frac{1}{1 + e^{-2{,}5}} = \frac{1}{1 + 0{,}0821} = \frac{1}{1{,}0821} = 0{,}924 \tag{3.34}
$$

**Langkah 3: _Loss_:**

$$
\mathcal{L} = -\left[w_p \cdot 0{,}975 \cdot \log(0{,}924) + 0{,}025 \cdot \log(1 - 0{,}924)\right] \tag{3.35}
$$

$$
= -\left[1 \times 0{,}975 \times (-0{,}0791) + 0{,}025 \times (-2{,}577)\right]
$$

$$
= -\left[-0{,}0771 + (-0{,}0644)\right] = -(-0{,}1415) = 0{,}1415
$$

Nilai _loss_ yang rendah (0,1415) menunjukkan bahwa prediksi model (probabilitas 0,924 untuk _fake_) sudah mendekati label target yang telah di-_smooth_ ($y' = 0{,}975$). Pada awal pelatihan, nilai _loss_ umumnya jauh lebih tinggi dan menurun seiring konvergensi model.
...
$$
\mathbf{g} \leftarrow \begin{cases} \mathbf{g} & \text{jika } \|\mathbf{g}\|_2 \leq 5{,}0 \\ \frac{5{,}0 \cdot \mathbf{g}}{\|\mathbf{g}\|_2} & \text{jika } \|\mathbf{g}\|_2 > 5{,}0 \end{cases} \tag{3.36}
$$
...
- **Kesabaran (_patience_):** 12 _epoch_ berturut-turut tanpa peningkatan AUC.
- **Jumlah _epoch_ maksimum:** 30.
- **Tindakan:** Pelatihan dihentikan dan _checkpoint_ terbaik digunakan untuk evaluasi.

Nilai _patience_ sebesar 12 _epoch_ dipilih untuk memberikan ruang transisi yang memadai di sekitar _epoch_ ke-4 ketika _backbone_ XceptionNet dilepaskan (_unfreezing_). Pelepasan _backbone_ kerap memicu degradasi performa validasi sementara karena gradien mulai mengalir ke seluruh jaringan dan distribusi fitur internal bergeser; dengan _patience_ 12 _epoch_, model memiliki kesempatan untuk beradaptasi dan memulihkan AUC validasi sebelum _early stopping_ dipicu. Selain itu, _counter_ _patience_ direset pada _epoch_ ke-4 (saat _unfreezing_) sehingga fase _fine-tuning_ menyeluruh memperoleh jendela waktu penuh untuk konvergensi.
...
| _Learning rate_ (_base_)        | $2 \times 10^{-4}$                | Lapisan baru (proyeksi, SE _gate_, _classifier_)     |
| _Learning rate_ (cabang freq, _hybrid_) | $5 \times 10^{-5}$       | FreqCNN dalam model _hybrid_ (base $\times$ 0,25)    |
...
| _Early stopping patience_       | 12                                | Tanpa peningkatan AUC                                |
| _Label smoothing_               | 0,05                              | Regularisasi ringan terhadap _overconfidence_        |
| _Pos weight_                    | $n_{\text{neg}} / n_{\text{pos}}$ | Penyeimbangan kelas                                  |
| _Gradient clipping_             | max_norm = 5,0                    | Norma L2                                             |
| _LR warmup_                     | 3 _epoch_                         | 10% → 100% linear, sinkron dengan _freeze_           |
| _LR schedule_                   | _Cosine annealing_                | Decay ke $1 \times 10^{-6}$                          |
| _Backbone freeze_               | 3 _epoch_                         | Semua model*pretrained*                              |
| _Mixed precision_               | AMP (_float16_)                   | CUDA saja                                            |
| FreqCNN _depth_ / _base_channels_ | 5 / 64                          | $\sim 4{,}2$ juta parameter; kanal $[64,128,256,512,512]$ |
| FFT _high-pass cutoff_ ($\beta$) | 0,15                             | Penekanan komponen frekuensi rendah                  |
| FFT _noise sigma_               | 0,05                              | Augmentasi Gaussian pada peta FFT                    |
| _Spectral band masking_         | $p = 0{,}05$, lebar $1$–$H/16$    | Augmentasi pita frekuensi acak                       |
...
$$
\Delta = \text{F1}_{\text{in-dataset}} - \text{F1}_{\text{cross-dataset}} \tag{3.38}
$$
