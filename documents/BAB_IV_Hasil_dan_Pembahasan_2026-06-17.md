...
|---|---|---|---|---|---|---|
| FFPP | spatial | 0,700 ± 0,010 | 0,666 ± 0,015 | 0,750 ± 0,023 | 0,705 ± 0,008 | **0,778 ± 0,010** |
| FFPP | hybrid | 0,600 ± 0,020 | 0,576 ± 0,028 | 0,644 ± 0,068 | 0,606 ± 0,014 | 0,644 ± 0,009 |
| FFPP | freq | 0,544 ± 0,015 | 0,525 ± 0,021 | 0,590 ± 0,122 | 0,550 ± 0,043 | 0,562 ± 0,007 |
| CDF | spatial | 0,913 ± 0,001 | 0,908 ± 0,013 | 0,904 ± 0,015 | 0,906 ± 0,002 | **0,971 ± 0,002** |
| CDF | hybrid | 0,847 ± 0,014 | 0,845 ± 0,026 | 0,824 ± 0,032 | 0,834 ± 0,016 | 0,919 ± 0,010 |
| CDF | freq | 0,552 ± 0,019 | 0,521 ± 0,021 | 0,503 ± 0,062 | 0,510 ± 0,034 | 0,562 ± 0,014 |

*Gambar 4.3 Perbandingan performa in-dataset ketiga model pada FFPP dan CDF (n = 750)*

Pada kedua dataset, model spasial memperoleh nilai tertinggi pada seluruh metrik, diikuti model *hybrid*, dan model frekuensi pada posisi terendah. Pada FFPP, AUC model spasial mencapai 0,778, sedangkan model *hybrid* dan frekuensi masing-masing 0,644 dan 0,562. Pola yang sama terjadi pada CDF dengan selisih yang lebih lebar, yaitu AUC 0,971 untuk model spasial, 0,919 untuk *hybrid*, dan 0,562 untuk frekuensi. Dataset CDF secara konsisten lebih mudah dideteksi dibanding FFPP bagi seluruh model. Nilai AUC model frekuensi yang hanya berada di kisaran 0,56 menunjukkan kemampuan diskriminasi yang hanya sedikit di atas tebakan acak (0,5). Urutan peringkat ini stabil, terlihat dari simpangan baku antar-*seed* yang kecil terutama pada model spasial.
...
*Gambar 4.4 Kurva ROC evaluasi in-dataset ketiga model (FFPP dan CDF, n = 750, seed 0)*
...
|---|---|---|---|---|---|---|
| FFPP→CDF | spatial | 0,627 ± 0,005 | 0,594 ± 0,012 | 0,637 ± 0,040 | 0,614 ± 0,013 | 0,678 ± 0,008 |
| FFPP→CDF | hybrid | 0,624 ± 0,012 | 0,599 ± 0,024 | 0,599 ± 0,119 | 0,594 ± 0,048 | 0,665 ± 0,016 |
| FFPP→CDF | freq | 0,541 ± 0,004 | 0,566 ± 0,018 | 0,064 ± 0,017 | 0,115 ± 0,028 | 0,606 ± 0,009 |
| CDF→FFPP | spatial | 0,554 ± 0,003 | 0,923 ± 0,020 | 0,074 ± 0,005 | 0,137 ± 0,009 | 0,607 ± 0,020 |
| CDF→FFPP | hybrid | 0,564 ± 0,006 | 0,736 ± 0,079 | 0,142 ± 0,011 | 0,238 ± 0,011 | 0,555 ± 0,031 |
| CDF→FFPP | freq | 0,558 ± 0,002 | 0,543 ± 0,018 | 0,531 ± 0,162 | 0,526 ± 0,085 | 0,575 ± 0,012 |

*Gambar 4.5 Perbandingan performa cross-dataset ketiga model pada kedua arah pengujian (n = 750)*

Dibandingkan evaluasi *in-dataset*, performa seluruh model menurun tajam pada skenario *cross-dataset*, dengan AUC berada pada kisaran 0,56–0,68. Penurunan paling mencolok tampak pada metrik *recall* di beberapa konfigurasi. Pada arah CDF→FFPP, model spasial mencatat presisi sangat tinggi (0,923) namun *recall* runtuh menjadi 0,074, yang berarti model nyaris tidak mengenali sampel *fake* dari dataset yang berbeda; pola serupa terjadi pada model *hybrid* (presisi 0,736, *recall* 0,142). Sebaliknya, pada arah FFPP→CDF, model frekuensi mengalami keruntuhan *recall* (0,064) sementara model spasial dan *hybrid* mempertahankan *recall* yang lebih wajar (0,637 dan 0,599). Kurva ROC untuk skenario *cross-dataset* ditunjukkan pada Gambar 4.6, memperlihatkan seluruh kurva yang jauh lebih dekat ke garis diagonal dibanding kurva *in-dataset*.

*Gambar 4.6 Kurva ROC evaluasi cross-dataset ketiga model (kedua arah, n = 750, seed 0)*
...
| hybrid | FFPP | 0,606 | 0,594 | **+0,012** |
| hybrid | CDF | 0,834 | 0,238 | +0,597 |
| freq | FFPP | 0,550 | 0,115 | +0,435 |
| freq | CDF | 0,510 | 0,526 | −0,015 |

*Gambar 4.7 Generalization drop F1-Score per model dan arah pelatihan (n = 750)*

Besarnya *generalization drop* sangat bergantung pada arah pelatihan. Pada model yang dilatih CDF, degradasi sangat besar, yaitu Δ = +0,769 untuk spasial dan Δ = +0,597 untuk *hybrid*. Sebaliknya, pada model yang dilatih FFPP, degradasi jauh lebih kecil, bahkan model *hybrid* mencatat Δ = +0,012 yang merupakan nilai terkecil di antara seluruh konfigurasi. Adapun model frekuensi menunjukkan Δ yang bervariasi, dari +0,435 (pelatihan FFPP) hingga negatif −0,015 (pelatihan CDF); nilai negatif ini perlu ditafsirkan dengan hati-hati karena performa *in-dataset* model frekuensi memang sudah rendah, sehingga praktis tidak ada performa yang dapat turun. Interpretasi atas pola ini diuraikan lebih lanjut pada subbab 4.2.
...
|---|---|---|
| RM1 — penurunan performa spasial lintas dataset | Terjadi penurunan substansial; AUC turun ke 0,607–0,678 dan *recall* runtuh hingga 0,074 (terparah pada arah CDF→FFPP) | Tabel 4.3, Tabel 4.4 |
| RM2 — pengaruh FFT terhadap penurunan | Memperkecil *generalization drop* pada arah FFPP→CDF (Δ hybrid +0,012 vs spasial +0,091), tetapi tidak konsisten dan tanpa peningkatan AUC menyeluruh, serta mengorbankan performa in-dataset | Tabel 4.3, Tabel 4.4 |
| RM3 — kontribusi spasial vs frekuensi | Spasial penyumbang utama (AUC s.d. 0,971); frekuensi nyaris acak (0,56–0,61) sehingga fusi *hybrid* tidak mengungguli spasial murni | Tabel 4.2, Gambar 4.4 |
